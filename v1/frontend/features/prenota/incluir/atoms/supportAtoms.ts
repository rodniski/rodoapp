...
import {AnexosCompleto, AnexoUpload, Produto, Rateio,} from "#/incluir/interfaces";
...
const isEditAction = (action: RateioAction): action is {
    type: "edit";
        ...
        switch (action.type) {
            ...
            case "edit":
                if (isEditAction(action)) {
                    set(rateiosAtom, currentRateios.map((r) =>
                        r.id === action.payload.id ? {...r, ...action.payload.updatedData} : r
                    ));
                }
                break;

            case "remove":
                set(rateiosAtom, currentRateios.filter((r) => r.id !== action.payload.id));
                break;
