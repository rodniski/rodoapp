...
import {Command, CommandGroup, CommandItem, CommandList, Input, Popover, PopoverContent, PopoverTrigger} from "ui";
...
const Search = () => {
        ...
        const handleBuscarXML = async () => {
         ...
         const validationResponse = await fetch(
             "http://172.16.99.174:8400/rest/PreNota/validaXML",
             {
                 method: 'GET',
                 headers: {'xml': xml.trim()}
             }
         );

                           const validationData = await validationResponse.json();

                           if (validationData.Incluido) {
                               toast.error(validationData.Mensagem);
                               return;
                           }
               */
                ...
                toast.promise(detalhesXmlQuery.refetch(), {
                    ...
                    success: (result) => {
                        if (result.data) {
                            ...
                            console.log("✅ xmlDataAtom atualizado:", result.data);
                            ...
                            const total = produtos.reduce((acc, produto) => acc + produto.TOTAL, 0);
                            setTotalGeral(total);

                            return "Busca concluída com sucesso.";
                        }
                        return "Erro ao buscar XML: Dados não encontrados.";
                    },
                    error: (err) => {
                        console.error("Erro ao buscar detalhes do XML:", err);
                        return "Erro ao buscar detalhes do XML.";
                    },
                });
            } catch
                (error) {
