...
import type { AuxState } from "@/app/login/_internal/types"; // Seus tipos
...
export const useAuxStore = create<AuxState>()( // Note o () extra aqui
  ...
  persist(
    ...
    (set, get) => ({
      ...
      loadInitialData: ({ filiais, condicoes, centroCusto }) =>
        set((state) => {
    ...
    // Opções de configuração do middleware 'persist'
    {
      ...
      onRehydrateStorage: (state) => {
        ...
        return (state, error) => {
          if (error) {
            console.log("useAuxStore: Falha ao reidratar!", error);
          }
        };
      },
      /**
