...
export function useFetchXmlDetails(): UseFetchXmlDetailsReturn {
  ...
  const fetchDetails = useCallback(
    async (chaveDFe: string): Promise<ConexaoNfeDetalhesResponse | null> => {
      ...
      catch(e) { console.log(`[${HOOK_NAME}] Erro ao limpar totalNf:`, e); }
      ...
      try {
        ...
        try {
            ...
            console.log(`[${HOOK_NAME}] Erro ao atualizar Stores Auxiliares pós-fetch:`, storeError);
        ...
        console.log(`[${HOOK_NAME}] Erro no fetchDetails para ${chaveDFe}:`, err);
        ...
        try { usePreNotaAuxStore.getState().totalNf.clearValorTotalXml(); }
        catch(e) { console.log(`[${HOOK_NAME}] Erro ao limpar totalNf no catch:`, e); }
        return null;
      } finally {
        setIsLoading(false);
      }
