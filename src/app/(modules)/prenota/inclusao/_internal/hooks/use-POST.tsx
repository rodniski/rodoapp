...
export function usePostPreNota() {
  ...
  return useMutation<PostPreNotaResponse, PostPreNotaHookError, void>({
    ...
    onError: (error: PostPreNotaHookError) => {
      console.log(
        `%c[${hookName}]%c ❌ Mutação Falhou! Causa: %c${error.cause || "unknown"}%c, Mensagem: %c${error.message}`,
        LOG_STYLES.hook,
        LOG_STYLES.error,
        LOG_STYLES.label,
        LOG_STYLES.error,
      ...
      // ---------- Parsing Protheus ----------
      let friendly = "";
      if (error.cause === "api_logic") {
        const p = parseProtheusError(error.message);
        switch (p.code) {
          case "AJUDAEXISTNF":
            friendly = `A nota fiscal **${p.nf}** (série ${p.serie}), emitida em ${p.emissao}, já está cadastrada para o fornecedor ${p.fornecedor}.`;
            break;
          // adicione outros códigos aqui se precisar
        }
      }
    
      toast.error(
        friendly || `Erro ao salvar: ${error.message || "Erro desconhecido"}`,
        { duration: 8000 }
      );
    },
