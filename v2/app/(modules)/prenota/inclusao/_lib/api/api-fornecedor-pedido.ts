...
export async function searchFornecedoresPedidosSimples(
    ...
    if (!busca || typeof busca !== 'string') {
        console.error("searchFornecedoresPedidosSimples: parâmetro 'busca' inválido:", busca);
    ...
    try {
        ...
        // Tratamento de erros.
        console.error(`[API FornecedorPedido] Falha ao buscar fornecedores (Busca: ${busca}):`, error);

        if (axios.isAxiosError(error) && error.response) {
            // Erro com resposta da API (status 4xx ou 5xx). Lança os dados do erro.
            console.error(`[API FornecedorPedido] Erro API - Status: ${error.response.status}`, error.response.data);
            throw error.response.data as FornecedorPedidoError;
        } else if (axios.isAxiosError(error) && error.request) {
            // Erro de requisição sem resposta (timeout, rede).
            console.error('[API FornecedorPedido] Erro Rede/Timeout:', error.request);
            throw new Error('Falha na comunicação com a API de Fornecedores.');
        } else {
            // Erro inesperado.
            console.error('[API FornecedorPedido] Erro Inesperado:', (error as Error).message);
