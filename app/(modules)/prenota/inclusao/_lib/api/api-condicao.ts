...
export async function getCondicaoPagamentoSimples(
    ...
    if (!params || typeof params !== 'object' || !params.valor || !params.condpag || !params.dtemissao || !params.codForn || !params.lojaForn) {
        console.error("getCondicaoPagamentoSimples: Parâmetros inválidos:", params);
    ...
    try {
        ...
        // Tratamento de erros.
        console.error(`[API CondicaoPagamento] Falha ao buscar condição (Params: ${JSON.stringify(params)}):`, error);

        if (axios.isAxiosError(error) && error.response) {
            // Erro com resposta da API (status 4xx ou 5xx). Lança os dados do erro.
            console.error(`[API CondicaoPagamento] Erro API - Status: ${error.response.status}`, error.response.data);
            throw error.response.data as CondicaoPagamentoError;
        } else if (axios.isAxiosError(error) && error.request) {
            // Erro de requisição sem resposta (timeout, rede).
            console.error('[API CondicaoPagamento] Erro Rede/Timeout:', error.request);
            throw new Error('Falha na comunicação com a API de Condição de Pagamento.');
        } else {
            // Erro inesperado.
            console.error('[API CondicaoPagamento] Erro Inesperado:', (error as Error).message);
