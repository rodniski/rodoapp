...
export async function postPedidoCompra(payload: PedidoCompraRequest): Promise<PedidoCompraResponse> {
  ...
  try {
    ...
    console.error(`${logPrefix} Erro na chamada da API:`, error);
    if (axios.isAxiosError(error)) {
      const axiosError = error as AxiosError;
      if (axiosError.response) {
        // A requisição foi feita e o servidor respondeu com um status code
        // que cai fora do range de 2xx
        console.error(`${logPrefix} Dados do erro da API:`, axiosError.response.data);
        console.error(`${logPrefix} Status do erro da API:`, axiosError.response.status);
        ...
        console.error(`${logPrefix} Nenhuma resposta recebida da API:`, axiosError.request);
        throw new Error('Nenhuma resposta recebida do servidor. Verifique a conexão ou o URL da API.');
      } else {
        // Algo aconteceu ao configurar a requisição que acionou um erro
        console.error(`${logPrefix} Erro ao configurar a requisição:`, axiosError.message);
